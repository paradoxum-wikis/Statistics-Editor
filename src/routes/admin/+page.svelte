...
{:else}
  <div class="flex h-screen flex-col bg-background">
    ...
    <main class="min-h-0 flex-1 overflow-y-auto p-5">
      ...
      <div class="overflow-x-auto rounded-[var(--radius)_0] border border-border">
          ...
          <tbody>
            {#each items as item (item.id)}
              <tr class="border-b border-border last:border-0">
                <td class="max-w-[12rem] truncate px-3 py-2 font-medium">
                  {item.title}
                  <div class="font-mono text-xs text-muted-foreground">
                    {item.id} · {item.share_id}
                  </div>
                </td>
                <td class="max-w-[8rem] truncate px-3 py-2">{item.tower_name}</td>
                <td class="max-w-[8rem] truncate px-3 py-2">
